...
def read_xml_and_generate_lp_with_weights(input_file, output_file):
    ...
    with open(output_file, "w") as lp_file:
        ...
        # for event in events:
        #     lp_file.write(f"""  {' + '.join([f'x_{event["id"]}_{time["id"]}' for time in times])} = {event['duration']}\n""")

        # # Restrições de sala: uma sala só pode ser usada por um evento por vez
        # for time in times:
        #     for room in rooms:
        #         lp_file.write(f"""  {' + '.join([f'x_{event["id"]}_{time["id"]}' for event in events])} <= 1\n""")
        ...
        #             constraint_name = f"Carga horária do evento {event['id']}"
        ...
        #         if terms:
        #             constraint_name = f"Conflito de horário do professor {teacher} no tempo {time['id']}"
        ...
        #         if terms:
        #             constraint_name = f"Conflito de horário da turma {cls} no tempo {time['id']}"
        #             lp_file.write(f" {constraint_name}: " + " + ".join(terms) + " <= 1\n")
        ...
        #             if terms:
        #                 constraint_name = f"Indisponibilidade do professor {constraint['id']} no tempo {time['id']}"
        ...
        #                 daily_terms[day] = []
        #             daily_terms[day].append(f"x_{event['teacher']}_{event['class']}_{time['id']}")
        #         for day, terms in daily_terms.items():
        #             if terms:
        #                 constraint_name = f"Máximo de aulas diárias do evento {event['id']} no dia {day}"
        ...
        #                 ]
        #                 if terms:
        #                     constraint_name = f"Lições duplas do evento {event['id']} no tempo {current_time}"
        ...
        #             terms = [
        #                 f"x_{teacher}_*_{current}",
        #                 f"x_{teacher}_*_{next_period}"
        #             ]
        #             if terms:
        #                 constraint_name = f"Período ocioso do professor {teacher} no tempo {current}"
        #                 lp_file.write(f" {constraint_name}: " + idle_var + " - " + " + ".join(terms) + " >= 0\n")

        # # S2: Dias de trabalho
        # for teacher in {e["teacher"] for e in events if e["teacher"]}:
        #     for day in ["Mo", "Tu", "We", "Th", "Fr"]:
        #         terms = [f"x_{teacher}_{time['id']}" for time in times if time["id"].startswith(day)]
        #         if terms:
        #             constraint_name = f"Dias de trabalho do professor {teacher} no dia {day}"
        ...
        #         if terms:
        #             constraint_name = f"Lições duplas solicitadas do evento {event['id']}"
        ...
        for event in events:
            for time in times:
                binary_vars.add(f"x_{event['id']}_{time['id']}")
            binary_vars.add(f"y_{event['teacher']}")
            binary_vars.add(f"z_{event['id']}")
